...
interface MobileAvailabilitySearchBarProps {
  checkIn: string | null;
...
export const MobileAvailabilitySearchBar = ({
  checkIn,
  checkOut,
  adults,
  children,
  ...
  onSearchChange,
  isLoading = false,
}: MobileAvailabilitySearchBarProps) => {
  ...
  return (
    <div className="flex flex-col gap-2 w-full">
      <div className="flex gap-2">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="outline"
              className={cn(
                "flex-1 justify-start text-left font-normal h-10 text-xs border",
        ...
        <Dialog open={showGuestPicker} onOpenChange={setShowGuestPicker}>
          <DialogTrigger asChild>
            <Button
              variant="outline"
              className="flex-1 justify-start text-left font-normal h-10 text-xs border"
            >
              ...
              {roomGuests.map((room, roomIndex) => (
                <div key={roomIndex} className="border rounded-lg p-4 space-y-4 bg-muted/20">
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-semibold">Room {roomIndex + 1}</div>
                    {roomGuests.length > 1 && (
                      <Button
                        variant="ghost"
                        size="sm"
      ...
      <Button 
        onClick={handleChangeSearch}
        className="w-full bg-primary hover:bg-primary/90 text-white h-10 text-sm"
        disabled={isLoading}
      >
